...
def Train(verbose, l1, l2, epoch, lr, batch_size, hidden_size, vocab_size, print_every, save_every, postfix, save_dir):
    ...
    for nE in range(epoch):
        ...
        pLoss = 0
        seq_len = 58
        for nB, (batch_index, batch_l, batch_sentence) in enumerate(l1_loader):
            if USE_CUDA:
            ...
            packed_sentence = pack_padded_sequence(
                batch_sentence, batch_l.cpu().numpy(), batch_first=True)
            ...
            for di in range(MAX_LENGTH):
                de_output, de_context, de_hidden, de_attention = l1_de(
                    de_input, de_context, de_hidden, output)
                choose, _ = torch.max(de_output)
                de_input = Variable(torch.FloatTensor(
                    l1_dataset.emb[l1_dataset.vocab.index2word[choose]]))
                if USE_CUDA:
                    de_input = de_input.cuda()
            ...
            if (nB + 1) % 20 == 0:
                print('Epoch %3d | Part %s | Batch %3d | loss %6.3f' %
                      (nE + 1, dataset.part, nB + 1, loss.data[0] / batch_index.size()[0]))
        ...
        for batch_index, batch_l, batch_sentence in enumerate(l2_loader):
            sen = Variable(sentence_swap(batch_index, batch_sentence))
            batch_l, indices = torch.sort(batch_l, dim=0, descending=True)
            batch_x = batch_sentence[indices]
            batch_x = Variable(batch_x).cuda() if USE_CUDA else Variable(batch_x)
            packed_x = pack_padded_sequence(batch_x, batch_l.numpy(), batch_first=True)
